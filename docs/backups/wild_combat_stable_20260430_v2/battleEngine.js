...
export const determineTurnOrder = (pMove, pPkmn, eMove, ePkmn) => {
  ...
  // Priority tie, use Speed (considering Paralysis)
  let pSpeed = pPkmn.speed || 10;
  let eSpeed = ePkmn.speed || 10;
  
  if (pPkmn.status === 'paralysis') pSpeed *= 0.5;
  if (ePkmn.status === 'paralysis') eSpeed *= 0.5;

  if (pSpeed > eSpeed) return ['player', 'enemy'];
  if (eSpeed > pSpeed) return ['enemy', 'player'];
 ...
 */
const getAccuracyMultiplier = (accStage = 0, evaStage = 0) => {
