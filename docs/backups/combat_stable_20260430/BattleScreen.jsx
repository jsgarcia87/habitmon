...
const BattleScreen = ({ navigate, battleData, aPressed }) => {
  ...
  const resolveTurn = async (pMoveKey) => {
    ...
    const pMove = MOVE_DATA[pMoveKey], eMoves = POKEMON_MOVES[Number(enemyPkmn.id)] || ['TACKLE'], eMove = MOVE_DATA[eMoves[Math.floor(Math.random()*eMoves.length)]];
    const order = determineTurnOrder(pMove, playerPkmn, eMove, enemyPkmn);
    for (const side of order) {
      if (playerPkmn.hp <= 0 || enemyPkmn.hp <= 0) break;
      const attacker = side === 'player' ? playerPkmn : enemyPkmn, defender = side === 'player' ? enemyPkmn : playerPkmn, move = side === 'player' ? pMove : eMove;
      const result = processMove(attacker, defender, move);
      setMessage(`¡${attacker.name.toUpperCase()} USÓ ${move.nombre}!`);
      ...
      if (result.damageDealt > 0) {
        ...
        if (side === 'enemy') setPlayerPkmn(prev => ({ ...prev, hp: Math.max(0, prev.hp - result.damageDealt) }));
        else setEnemyPkmn(prev => ({ ...prev, hp: Math.max(0, prev.hp - result.damageDealt) }));
        activeAnimsRef.current.push(createAnimation(ANIM_TYPES.TEXT, { text: `-${result.damageDealt}`, x: side === 'enemy' ? 300*0.25 : 300*0.75, y: side === 'enemy' ? 200*0.7-20 : 200*0.35-20, color: '#f00', duration: 800 }));
      }
      await new Promise(r => setTimeout(r, 1200));
    }
    if (playerPkmn.hp > 0 && enemyPkmn.hp > 0) { setPhase(BATTLE_PHASES.SELECT); setMessage(''); }
    else endBattle(enemyPkmn.hp <= 0);
  };
  ...
  const endBattle = async (win) => {
    ...
    if (!win) {
      ...
      activeAnimsRef.current.push(createAnimation(isWild ? ANIM_TYPES.CAPTURE : ANIM_TYPES.FAINT, { target: 'enemy', targetX: 300*0.73, targetY: 200*0.38, duration: 2500 }));
