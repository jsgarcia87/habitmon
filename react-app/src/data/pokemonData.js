...
export const createBattlePokemon = (speciesId, level, customName = null) => {
  ...
  // Simple but faithful stat formula
  const maxHp = Math.floor((base.hp * level / 25) + level + 10);
  const atk = Math.floor((base.atk * level / 50) + 5);
  const def = Math.floor((base.def * level / 50) + 5);
  const spa = Math.floor((base.spa * level / 50) + 5);
  const spd = Math.floor((base.spd * level / 50) + 5);
  const spe = Math.floor((base.spe * level / 50) + 5);
